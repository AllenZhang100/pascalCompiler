...
{ TOKEN idtok, typetok, idlist, rectypes, next,front,sub;
  idlist = NULL;
  front = NULL;
  SYMBOL type, rec, fields, rectype,cur, subsym;
  int size, total, rectotal = 0;
  while(peektok()->tokentype == IDENTIFIERTOK)
  {
    idtok,keytok,typetok = NULL;
    ...
    if(keytok->tokentype!= OPERATOR || keytok->whichval!=EQOP)
       yyerror("Invalid constant declaration");
     typetok = gettok();
     if(typetok->tokentype == RESERVED && typetok->whichval + RESERVED_BIAS == RECORD)
     {
          type = searchins(idtok->stringval);
          type->kind = TYPESYM;
          ...
          rec->datatype = NULL;
          type->datatype = rec;
        while(peektok()->whichval + RESERVED_BIAS != END)
        { 
          keytok = gettok();
          if(front == NULL)
            front = keytok;
          ...
          if(peektok()->whichval + DELIMITER_BIAS == COMMA)
            gettok();
          else if(peektok()->whichval + DELIMITER_BIAS == COLON)
          {
            gettok();  // Get the colon
            rectypes = gettok();
            rectype = searchst(rectypes->stringval);
            while(front!=NULL)
            {

              fields = searchins(front->stringval);
              fields->size = rectype->size;
              fields->offset = rectotal;
              fields->datatype = rectype;
              rectotal= rectotal +rectype->size;
              front = front->link;
          ...
          gettok(); //get rid of end
          rec->size = wordaddress(rectotal,16);
          type->size = wordaddress(rectotal,16);
      }
     /* Subrange case */
     // else if(typetok->tokentype == DELIMITER && typetok->whichval + DELIMITER_BIAS == LPAREN)
     // {
     //    type->size = 4;  //subranges are always of size 4
